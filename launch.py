...
from swisscom_ai.research_keyphrase.embeddings.emb_distrib_doc2vec import (
    EmbeddingDistributorDoc2Vec,
)

from swisscom_ai.research_keyphrase.embeddings.emb_distrib_sentencetransformers import (
    EmbeddingDistributorSBert,
)
...
def extract_keyphrases(
    embedding_distrib, ptagger, raw_text, N, lang, beta=0.55, alias_threshold=0.7
):
    """
    ...
    return MMRPhrase(
        embedding_distrib, text_obj, N=N, beta=beta, alias_threshold=alias_threshold
    )
...
def load_doc2vec_embedding_distributor():
    config_parser = ConfigParser()
    config_parser.read("config.ini")
    sent2vec_model_path = config_parser.get("DOC2VEC", "model_path")
    return EmbeddingDistributorDoc2Vec(sent2vec_model_path)


def load_sbert_embedding_distributor():
    return EmbeddingDistributorSBert()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-N", help="number of keyphrases to extract", required=True, type=int
    )
